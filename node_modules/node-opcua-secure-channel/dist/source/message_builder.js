...
class MessageBuilder extends node_opcua_transport_1.MessageBuilderBase {
    constructor(options) {
        ...
        node_opcua_assert_1.assert(typeof this.objectFactory.constructObject === "function", " the objectFactory must provide a constructObject method");
        this._previousSequenceNumber = -1; // means unknown
        node_opcua_assert_1.assert(isFinite(this._previousSequenceNumber));
        this._tokenStack = [];
    }
    ...
    _read_headers(binaryStream) {
        ...
        try {
            ...
            if (msgType === "HEL" || msgType === "ACK") {
                ...
                if (doTraceChunk) {
                    console.log(node_opcua_utils_1.timestamp(), chalk.red("   >$$ "), chalk.red(this.messageHeader.msgType), chalk.red("nbChunk = " + this.messageChunks.length.toString().padStart(3)), chalk.red("totalLength = " + this.totalMessageSize.toString().padStart(8)), "l=", this.messageHeader.length.toString().padStart(6), errorCode.toString(), message);
                }
                return true;
            }
            else {
    ...
    _decodeMessageBody(fullMessageBody) {
        ...
        let id;
        try {
            id = node_opcua_basic_types_1.decodeExpandedNodeId(binaryStream);
        }
        catch (err) {
    ...
    _validateSequenceNumber(sequenceNumber) {
        ...
        node_opcua_assert_1.assert(isFinite(this._previousSequenceNumber));
        node_opcua_assert_1.assert(isFinite(sequenceNumber) && sequenceNumber >= 0);
    ...
    _decrypt_OPN(binaryStream) {
        ...
        if (doTraceChunk) {
            const thumb = node_opcua_crypto_1.makeSHA1Thumbprint(asymmetricAlgorithmSecurityHeader.senderCertificate).toString("hex");
            console.log(node_opcua_utils_1.timestamp(), ` >$$ securityPolicyId:  ${asymmetricAlgorithmSecurityHeader.securityPolicyUri} ${thumb} `);
        }
        ...
        node_opcua_assert_1.assert(signatureLength === 128 || signatureLength === 256 || signatureLength === 384 || signatureLength === 512);
