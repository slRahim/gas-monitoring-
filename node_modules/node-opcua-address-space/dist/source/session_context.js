...
function getPermissionForRole(rolePermissions, role) {
    ...
    const a = rolePermissions.find((r) => {
        return node_opcua_nodeid_1.sameNodeId(node_opcua_nodeid_1.resolveNodeId(r.roleId), role);
    });
...
function getDefaultUserRolePermissionsOnNamespace(namespace, context) {
    ...
    if (defaultUserRolePermissions) {
        const dataValue = defaultUserRolePermissions.readAttribute(null, node_opcua_data_model_1.AttributeIds.Value);
    ...
    if (defaultRolePermissions) {
        const dataValue = defaultRolePermissions.readAttribute(null, node_opcua_data_model_1.AttributeIds.Value);
        if (dataValue && dataValue.statusCode === node_opcua_status_code_1.StatusCodes.Good) {
            return dataValue.value.value;
        }
    }
...
function makeRoles(roleIds) {
    ...
    if (typeof roleIds === "string") {
        roleIds = roleIds.split(";").map(r => node_opcua_nodeid_1.resolveNodeId("WellKnownRole_" + r));
    }
    return roleIds.map((r) => node_opcua_nodeid_1.resolveNodeId(r));
}
...
class SessionContext {
    ...
    getCurrentUserRoles() {
        ...
        if (rolesNodeId.findIndex((r) => r.namespace === 0 && r.value === WellKnownRoles.AuthenticatedUser) < 0) {
            rolesNodeId.push(node_opcua_nodeid_1.resolveNodeId(WellKnownRoles.AuthenticatedUser));
        }
        return rolesNodeId;
    }
    ...
    currentUserHasRole(role) {
        ...
        const n = node_opcua_nodeid_1.resolveNodeId(role);
        return currentUserRole.findIndex(r => node_opcua_nodeid_1.sameNodeId(r, n)) >= 0;
    }
}
