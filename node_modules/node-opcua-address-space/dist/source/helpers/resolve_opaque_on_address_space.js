...
function resolveOpaqueOnAddressSpace(addressSpace, variants) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const session = new __1.PseudoSession(addressSpace);
        const extraDataTypeManager = yield __1.ensureDatatypeExtracted(addressSpace);
        if (variants instanceof node_opcua_variant_1.Variant) {
            yield node_opcua_client_dynamic_extension_object_1.resolveDynamicExtensionObject(session, variants, extraDataTypeManager);
            return;
        }
        // resolve opaque data structure from inputArguments
        for (const variant of variants) {
            if (variant) {
                yield node_opcua_client_dynamic_extension_object_1.resolveDynamicExtensionObject(session, variant, extraDataTypeManager);
            }
        }
    });
}
